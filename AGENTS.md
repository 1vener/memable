...
- 115 网盘补齐 SHA1：**自研** `internal/pan115`（参考 SheltonZhu/115driver 的 webapi，零第三方依赖）——`LoginCheck`/`ListDir`（目录树懒加载）/`WalkDir`（递归+分页+sha1/大小映射）；风控内置：串行 + `netdrive.request_interval_ms`（默认 1000ms）+ jitter（+500ms/-200ms，实际 800~1500ms）+ 指数退避 + `state=false`/`errno` 区分"Cookie 失效"与"触发风控"即停。**独立任务队列**（第三队列 `netdriveKinds`/`DequeueNextNetdrive`/`netdriveWake`/`netdriveLoop`，主队列 `DequeueNext` 同时排除报告与网盘两类）。Cookie 存 `settings` 表（`repo.SettingsKeyNetdriveCookie`，设置页填写/验证/保存，`POST /api/netdrive/115/verify`）；目录树右键"从 115 补齐 SHA1" → 弹窗选网盘目录（`GET /api/netdrive/115/tree`）→ 提交任务（`POST /api/netdrive/115/sync-sha1`，kind=`netdrive_sha1`）→ 网盘遍历拉映射 → 本地 `ListMissingSha1ByDirectory`（**注意：该查询必须带 file_size 列供大小校验，曾因只查 id/relative_path 导致全部误判大小不符**）→ 相对路径对齐 + 大小校验（`netdrive.match_size`）→ 逐条 UpdateSha1 → 结果 `{matched,mismatched,not_found,failed}`。未命中不自动本地计算（本地算走原 scan_sha1）。
