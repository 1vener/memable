...
- 数据库迁移：`internal/db` 内嵌 `schema.sql` 与 `migrations/`（v2=重复报告三表；v3=background_tasks.kind 增加 report_duplicate；v4=background_tasks.kind 增加 scan_sha1；v5=media 表新增 scan_session_id/kind 与 kind/created_at 索引；v6=background_tasks.kind 增加 report_directory + 目录对比报告独立三表；v7=media 表新增 cover_phash 列；v8=settings 杂项参数表 + background_tasks.kind 增加 netdrive_sha1），当前 `schema_version=8`。新表结构变更需新增迁移文件并在 `internal/db/db.go` 的 `steps` 注册；修改带 CHECK 约束的表需重建表迁移（参照 003/004/006/008），并同步内嵌 schema.sql 与根目录 schema.sql。`ALTER TABLE` 类迁移（如 v7 加列）需在 steps 里配 `skip`（`columnExists`）做幂等保护：新库 schema.sql 已含该列，仅存量库真正执行。
- 115 网盘补齐 SHA1：**经本地 CloudDrive2（CD2）gRPC 官方 API**（`internal/cd2`，官方 clouddrive.proto v1.0.13 生成代码在 `internal/cd2/proto`，零手写 wire 协议）——`Ping`（GetSystemInfo）/`VerifyToken`（GetApiTokenInfo 校验 token + `allow_list` 权限）/`ListDirs`（目录树懒加载，路径为 CD2 完整路径如 `/115/视频`）/`WalkDir`（递归遍历 GetSubFiles 服务端流，取 `CloudDriveFile.fileHashes[HashType.Sha1]` + size + isDirectory + fullPathName）。**全局限速**（`cd2.SetRateLimit`，进程级单例，验证/目录树/后台任务共享，防并发绕过）：串行 + `netdrive.request_interval_ms`（默认 1000ms）+ jitter（+500ms/-200ms，实际 800~1500ms，`Int63n(701)-200`）+ 指数退避（重试重新限速）+ `Unavailable` 才重试、`Unauthenticated/PermissionDenied`→`ErrInvalidToken` 即停。**独立任务队列**（第三队列 `netdriveKinds`/`DequeueNextNetdrive`/`netdriveWake`/`netdriveLoop`，主队列 `DequeueNext` 同时排除报告与网盘两类）。CD2 地址/Token 存 `settings` 表（`repo.SettingsKeyNetdriveAddr/Token`，设置页填写/验证/保存，`POST /api/netdrive/cd2/verify`）；目录树右键"网盘 → 115 网盘 → CloudDrive2 → 从 CD2 补齐 SHA1"（`context_menu.dart` 支持多级子菜单）→ 弹窗选网盘路径（`GET /api/netdrive/cd2/tree?path=`）→ 提交任务（`POST /api/netdrive/cd2/sync-sha1`，kind=`netdrive_sha1`，payload `remote_path`）→ CD2 遍历拉映射 → 本地 `ListMissingSha1ByDirectory`（**注意：该查询必须带 file_size 列供大小校验，曾因只查 id/relative_path 导致全部误判大小不符**）→ 相对路径对齐 + 大小校验（`netdrive.match_size`）→ 逐条 UpdateSha1 → 结果 `{matched,mismatched,not_found,failed}`。未命中不自动本地计算（本地算走原 scan_sha1）。**已放弃 115 WebAPI**（`internal/pan115` 已删除，不再存 Cookie、不再直连 webapi.115.com）。测试用 bufconn 假 CD2 gRPC 服务 + `cd2.SetDialer`（`passthrough:///` 拨号）。
...
- HTTP 端口：默认 **12358**，优先级 `-port` flag > env `MEMABLE_SERVER__PORT` > `config.yaml` `server.port` > 默认；**被占用自动 +1 避让（上限 20 次）**，`port=0` 随机端口；实际端口经 `GET /api/settings` 的 `server_port` 与启动日志暴露（`Server.ActualPort()`）。前端 `main.dart` 启动时探测 12358~12377 的 `/api/health`，**仅 release 构建**全失败时经 `BackendManager` 自动拉起同目录 `server.exe`（`Process.start` + 轮询就绪），**退出清理走 `AppLifecycleListener.onExitRequested`（官方钩子，先 `stopBackend` 再退出）+ dispose 双保险**；debug 模式不自动拉起（开发时手动起服务）。按用户决定**不做后端孤儿进程兜底**：前端崩溃/强杀时 server.exe 可能残留，下次启动探测到端口通自动复用。打包发布：`Build-Release.ps1` 一键构建前后端（Go `-X main.version` 注入版本、强制 amd64、Flutter release）并压缩到 `dist/memable-<version>-windows-amd64.zip`；GitHub Actions `.github/workflows/release.yml` 打 tag `v*` 自动编译打包发布 Release。
...
- 数据库：DSN 一律用 `_pragma` 形式设置 `journal_mode(WAL)`、`foreign_keys(1)`、`busy_timeout`、`synchronous(NORMAL)`、`cache_size`、`mmap_size`；**modernc 驱动只认 `_pragma`，裸参数（如 `_journal_mode=WAL`）会被静默忽略**。媒体入库为单条 `Upsert`（`INSERT ... RETURNING id`，无批量写入）。`database.path` 默认两态（config.Load 内）：显式配置（yaml/env，`v.InConfig` 判定，**勿用 `v.IsSet`——SetDefault 默认值也会被它算作已设置**）→ 按配置；否则 → 系统数据目录（`config.dataRootDir()/memable/memable.db`）。**无"存量库沿用"兼容**：注释掉配置即迁移到系统目录，原位置存量文件由用户手动迁移。`db.Open` 自动 `MkdirAll` 数据库父目录（系统目录默认父目录不存在）。
...
- 目录对比报告（`dir_duplicate_reports/groups/members` 独立三表）：**独立页面** `flutter_app/lib/screens/dir_compare_screen.dart`（侧边栏"目录对比"入口，Ctrl+6），与重复报告页分离；"开始/重新生成"按钮 → 选收藏库 + 目录（含子目录）→ 任务 kind=`report_directory`（报告队列）→ `Service.GenerateDirCompare`；检测复用 queryMask 机制（scope=`dir_vs_rest`，目标=所选目录媒体，索引全量），SHA1/OSHash/pHash 组必须**同时含目标与存量**（目标内部、存量内部不比较），成员带 `is_target` 标记；不替换重复报告，重新生成替换自身。**结果展示与操作与重复报告页完全一致**（目录树[顶部搜索框，实时过滤/高亮/自动定位] + 重复分组双视图 + 媒体类型过滤 + 分页 10/20/50/100 + card_swiper 叠卡[目录树叠卡展示整组成员：本目录成员优先、其它目录成员补充] + 右键菜单[打开/复制/打开/排除重复/删除此文件/其它目录二级菜单/删除此文件外本组重复文件] + 一键清除[目录/本页 + 六种保留条件，**目录清除语义与重复报告页一致**（`Service.DirClear` scope=directory 按本目录成员处理、只删本目录数据）] + 组详情弹窗[成员缩略图 hover 显示完整路径]），成员卡片/叠卡带 `is_target`"所选目录"角标；后端接口：`GET .../directory-compare/tree`（kind 过滤）、`GET .../groups`（kind 过滤）、`POST .../exclude`（`DirExcludeMedia`）、`POST .../clear`（`DirClear`，scope=directory/page/group + keep）；`PruneAfterMediaChange` 同时刷新普通与目录对比两张报告（`Dir.PruneGroupsAndUpdateStats` 清 <2 组 + 重算统计）。前端公共组件（`StackedCluster`/`LazyThumb`/`DirChip`/`DuplicateMemberCard`[menuBuilder 参数化、showPathTooltip]/`showKeepDialog`/`confirmClearDialog`/`pickKeepIndex`/工具函数）统一在 `widgets/report_common.dart`，两个报告页共用，勿在页面内复制私有副本。
...
- 分组视图：平铺分页（默认 20 组/页，可切 10/20/50/100）；右键菜单（打开/复制/打开媒体、**排除重复**（`POST /api/reports/duplicate/exclude`，删除该文件全部组成员关系并清理 <2 的组、刷新统计，仅当前报告生效）、**删除此文件**（独立确认后 `POST /api/media/delete` 删单个文件，同步删本地文件/media 记录/缩略图）、其它目录二级菜单、删除此文件外本组重复文件）；一键清除（目录/整页 + 六种保留条件）；删除后局部刷新，空组不再展示。**目录清除语义（scope=directory，后端 `Service.Clear` 与前端 `_computeDirDeletedIds` 一致）**：按"本目录成员"为单位处理、只删本目录数据——组内本目录成员 ≥2 按保留条件保留 1 个删其余；组内本目录成员 ==1（仅跨目录重复）直接删除本目录这一份；其它目录成员一律不动。目录树面板顶部有搜索框（`_onTreeSearch` 过滤 + 高亮 + 自动选中首个匹配）；组详情弹窗缩略图 `showPathTooltip` hover 显示完整路径。
...
- 缩略图路径：`media.thumbnail_path` 只存相对"该类型缩略图根目录"的内容寻址路径（`{key[:2]}/{key}.jpg`，不含类型前缀）；根目录默认系统数据目录（Windows `%LOCALAPPDATA%`、macOS `~/Library/Application Support`、Linux `$XDG_DATA_HOME` 或 `~/.local/share` 下 `memable/thumbnails/{image,video}`，`config.dataRootDir()` 统一解析），`config.yaml` 的 `thumbnail.image_dir/video_dir` 可覆盖；静态访问 `/api/thumbnails/{kind}/{rel}`，整体移动缩略图目录只需改配置无需改库。缩略图为 400px（`thumbnail.max_edge`）JPEG q90（CatmullRom 缩放，`x/image/draw`；旧版为 300px PNG 最近邻——`scan` 的 `needRepairFromSnapshot` 把 `.png` 后缀视为旧格式，完整性扫描时重生为 `.jpg` 并清理无引用旧文件，勿改回 PNG）。**默认目录已从系统缓存改为系统数据目录**：旧版缩略图在缓存目录（macOS `~/Library/Caches`、Linux `$XDG_CACHE_HOME`），升级后由用户手动迁移或配置指向旧位置。
...
- 打包/运行踩坑（Build-Release.ps1 与发布包）：① PowerShell 5.1 下勿用 `$ErrorActionPreference="Stop"`——外部命令（go/flutter/git，尤其 `.bat` 包装的 flutter）的 stderr/部分 stdout 会被包装成 NativeCommandError 终止脚本，应外部命令显式检查 `$LASTEXITCODE`、cmdlet 用 `-ErrorAction Stop`；② `Copy-Item "src\*" "dest\"` 在目标目录不存在时会摊平子目录——Flutter Release 的 `data/`（含 `app.so` AOT 数据）必须整目录复制并校验 `data/app.so` 存在，否则 memable.exe 报 "Can't load AOT data" 直接闪退；③ viper v1.21 起 `ReadInConfig` 文件不存在返回原始 `*os.PathError` 而非 `ConfigFileNotFoundError`，config.Load 容错需加 `os.IsNotExist` 判断；④ 打包时 `config.yaml` 必须与 `server.exe` 同目录（前端以相对路径 `-config config.yaml` 拉起，双击启动工作目录即 app/）。**无黑窗口**：发布构建 server 加 `-ldflags "-H windowsgui"`（GUI 子系统，前端拉起不弹控制台黑窗）；所有 exec 子进程（ffmpeg/ffprobe/PowerShell）经 `internal/cmdx.Command` 统一设置 `SysProcAttr.HideWindow`（CREATE_NO_WINDOW），否则 GUI 宿主启动 console 程序会弹黑窗；GUI 构建下 `os.Stdout.Stat()` 报错（handle invalid），logx 检测后自动回退写 `dataRootDir()/memable/server.log`（开发用普通 console 构建看控制台日志）。
