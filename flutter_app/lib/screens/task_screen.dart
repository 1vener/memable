...
class _TaskScreenState extends State<TaskScreen> {
  ...
  List<BackgroundTask> get _runningTasks => _tasks.where((t) => t.isRunning).toList();
  List<BackgroundTask> get _queuedTasks => _tasks.where((t) => t.isQueued).toList();
  List<BackgroundTask> get _historyTasks => _tasks.where((t) => t.isTerminal).toList();
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('取消任务', style: TextStyle(color: Color(0xFFEF4444))),
          ),
        ],
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('取消失败: $e'), backgroundColor: const Color(0xFFEF4444)),
        );
      }
    }
  }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text('任务队列', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: cs.onSurface)),
              const Spacer(),
              if (hasActive)
                Container(
            ...
            child: _tasks.isEmpty
                ? Center(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(Icons.check_circle_outline, size: 64, color: cs.outline.withValues(alpha: 0.5)),
                        const SizedBox(height: 16),
                        Text('暂无任务', style: TextStyle(fontSize: 15, color: cs.outline)),
                        const SizedBox(height: 8),
                        Text('扫描、修复或报告任务会在此显示', style: TextStyle(fontSize: 13, color: cs.outline)),
                      ],
                    ),
                  )
                : ListView(
                    children: [
                      ...
                      if (_runningTasks.isNotEmpty) ...[
                        _SectionHeader(title: '运行中', icon: Icons.play_circle, color: cs.primary),
                        const SizedBox(height: 8),
                        for (final task in _runningTasks) _TaskCard(task: task, onCancel: () => _cancelTask(task)),
                        const SizedBox(height: 20),
                      ],
                      ...
                      if (_historyTasks.isNotEmpty) ...[
                        _SectionHeader(title: '历史', icon: Icons.history, color: cs.outline),
                        const SizedBox(height: 8),
                        for (final task in _historyTasks) _TaskCard(task: task),
                      ],
                    ],
                  ),
          ),
...
class _TaskCard extends StatelessWidget {
  ...
  Color _statusColor(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    switch (task.status) {
      case 'running': return cs.primary;
      case 'queued': return const Color(0xFFF59E0B);
      case 'completed': return const Color(0xFF22C55E);
      case 'failed': return cs.error;
      case 'cancelled': return cs.outline;
  ...
  IconData _kindIcon() {
    switch (task.kind) {
      ...
      case 'temporary_scan': return Icons.flash_on;
      case 'report_image': return Icons.photo_library;
      case 'report_video': return Icons.movie;
      case 'promote': return Icons.move_up;
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(task.title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: cs.onSurface)),
                ...
                Container(
                  ...
                  child: Text(task.statusLabel, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: statusColor)),
                ),
                // 取消按钮
                if (onCancel != null) ...[
                  const SizedBox(width: 8),
            ...
            if (task.isRunning && task.totalItems > 0) ...[
              ...
              Row(
                children: [
                  _ProgressChip(label: '已处理', value: task.processedItems, color: cs.primary),
                  const SizedBox(width: 8),
                  _ProgressChip(label: '成功', value: task.succeededItems, color: const Color(0xFF22C55E)),
                  const SizedBox(width: 8),
                  _ProgressChip(label: '跳过', value: task.skippedItems, color: const Color(0xFFF59E0B)),
                  const SizedBox(width: 8),
                  _ProgressChip(label: '失败', value: task.failedItems, color: cs.error),
                  const Spacer(),
                  Text(
                    '共 ${task.totalItems} 项',
                    style: TextStyle(fontSize: 12, color: cs.outline),
...
class _ProgressChip extends StatelessWidget {
  ...
  final int value;
  final Color color;
  const _ProgressChip({required this.label, required this.value, required this.color});
